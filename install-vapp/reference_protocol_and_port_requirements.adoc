...
NOTE: La porta predefinita per MySQL, 3306, è limitata solo all'host locale durante l'installazione di Unified Manager su sistemi VMware vSphere. Questo non influisce su nessuno scenario di aggiornamento, in cui viene mantenuta la configurazione precedente. Questa configurazione può essere modificata e la connessione può essere resa disponibile ad altri host utilizzando `Control access to MySQL port 3306` sulla console di manutenzione.
...
[NOTE]
====
Le porte utilizzate per le comunicazioni HTTP e HTTPS (porte 80 e 443) possono essere modificate utilizzando la console di manutenzione di Unified Manager. Per ulteriori informazioni, consultare la sezione _Configurazione di Active IQ Unified Manager_.

====
link:../config/concept_configure_unified_manager.html["Configurazione di Active IQ Unified Manager"]
...
NOTE: Se si utilizza questa porta o qualsiasi altra porta per connettersi a VMware vCenter Server o al server ESXi, assicurarsi che la porta sia disponibile e possa essere collegata in un sito protetto.
